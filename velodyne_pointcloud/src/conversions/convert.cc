...
namespace velodyne_pointcloud
{
  ...
  Convert_T::Convert_T(ros::NodeHandle node, ros::NodeHandle private_nh):
    data_(new velodyne_rawdata::RawData()), first_rcfg_call(true)
  {
    ...
    dynamic_reconfigure::Server<velodyne_pointcloud::CloudNodeConfig>::
      CallbackType f;
    f = boost::bind (&Convert_T::callback, this, _1, _2);
    srv_->setCallback (f);

    // subscribe to VelodyneScan packets
    velodyne_scan_ =
      node.subscribe("velodyne_packets", 10,
                     &Convert_T::processScan, (Convert_T *) this,
                     ros::TransportHints().tcpNoDelay(true));
  ...
  void Convert_T::callback(velodyne_pointcloud::CloudNodeConfig &config,
                uint32_t level)
  {
    ROS_INFO("Reconfigure Request");
    data_->setParameters(config.min_range, config.max_range, config.view_direction,
  ...
  void Convert_T::processScan(const velodyne_msgs::VelodyneScan::ConstPtr &scanMsg)
  {
    if (output_.getNumSubscribers() == 0)         // no one listening?
      return;                                     // avoid much work
    ...
    for (size_t i = 0; i < scanMsg->packets.size(); ++i)
    {
      data_->unpack_T(scanMsg->packets[i], *container_ptr_);
    }
